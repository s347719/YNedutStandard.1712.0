...
public class PresencePacketListenerImpl implements PacketListener {
	...
	public void processPacket(Packet arg0) {
		...
		if (xNameSpace == null) {
			...
			String resouce = JIDUtil.getResouceNameByJID(p.getFrom().trim());
			User tempUser = mContactOrgDao.queryUserInfoByUserNo(mUserNo);
			if (tempUser == null) {
                return;
            }
			if (p.getType() == Presence.Type.available) {
				if (tempUser.getUserStatus() == Const.USER_OFF_LINE) {
					if (resouce.equals("Msg_Phone")) {
						mContactOrgDao.updateOneUserStatusByAble(mUserNo,Const.USER_ON_LINE_PHONE);
					} else {
						mContactOrgDao.updateOneUserStatusByAble(mUserNo,Const.USER_ON_LINE_PC);
					}
				} else {
					if (tempUser.getUserStatus() == Const.USER_ON_LINE_PHONE && resouce.equals("Msg_PC")) {
						mContactOrgDao.updateOneUserStatusByAble(mUserNo,Const.USER_ON_LINE_ALL);
					} else if (tempUser.getUserStatus() == Const.USER_ON_LINE_PC && resouce.equals("Msg_Phone")) {
						mContactOrgDao.updateOneUserStatusByAble(mUserNo,Const.USER_ON_LINE_ALL);
					}
				}
...
//				mContactOrgDao.updateOneUserStatusByAble(mUserNo,Const.USER_ON_LINE);
				...
				if (tempUser.getUserStatus() == Const.USER_ON_LINE_ALL) {
					if (resouce.equals("Msg_Phone")) {
						mContactOrgDao.updateOneUserStatusByAble(mUserNo,Const.USER_ON_LINE_PC);
					} else {
						mContactOrgDao.updateOneUserStatusByAble(mUserNo,Const.USER_ON_LINE_PHONE);
...
//			notify();
	 ...
	 * @param mFromGroupMemberAccount 来自组里的成员帐号
	...
	private void quitGroupByGroupType(String mGroupName, String mFromGroupMemberAccount, int mGroupType) {
		...
		if (AppController.getInstance().mSelfUser != null &&
				mFromGroupMemberAccount.equals(AppController.getInstance().mSelfUser.getUserNo())) {
			//删除讨论组会话里面的聊天记录
			mDisGroupChatDao.deleteRecentChatByGroupId(mGroupName);
			//删除消息列表的聊天记录
			mRecentChatDao.deleteRecentChatByUserNo(mGroupName);
...
//	}
	public interface groupCreatedListener{
		void groupCreated(String mGroupName);
	}

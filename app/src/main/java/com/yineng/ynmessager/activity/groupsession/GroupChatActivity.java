 ...
 * @author YINENG
 */
public class GroupChatActivity extends BaseChatActivity implements OnClickListener, ReceiveMessageCallBack, ReceiveReqIQCallBack, FaceRelativeLayout.OtherButtonsClickListener, AdapterView.OnItemLongClickListener {
    public static final String CHAT_ID_KEY = "CHAT_GROUP_ID";
    public static final String CHAT_TYPE_KEY = "CHAT_GROUP_TYPE";
     ...
     * 未读消息列表
     */
    List<GroupChatMsgEntity> mUnreadList = new ArrayList<GroupChatMsgEntity>();
                ...
                case GET_RECEIPT:// 获得回执
                    ...
                    if (entity != null
                        ...
                        if (entity.getPacketId().equals(reqIq.getPacketID())) {
                            mRecentChatDao.updateRecentChatTime(reqIq.getSendTime(), mChatUserNum, Const.CHAT_TYPE_GROUP);
                        }
                        // updateMessageList(entity);
                        mGroupChatDao.saveOrUpdate(entity);
                    }
    ...
    private RelativeLayout mCommonHeadViewRL;
    ...
    public boolean onItemLongClick(AdapterView<?> parent, View view, int position, long id) {
        {
            ...
            new TipView.Builder(GroupChatActivity.this, layout_chat, (int) OldListX + view.getWidth() / 2, (int) OldListY)
                    ...
                    .setOnItemClickListener(new TipView.OnItemClickListener() {
                        ...
                        public void onItemClick(String str, final int position) {
                            if (position == 0) {
                                mClipboard.setPrimaryClip(ClipData.newPlainText(mTag, viewItem.getSpannableString().toString()));
                                if (viewItem.getMessageType() == 0 || viewItem.getMessageType() == 1) {
                                ...
                                if (viewItem.getIsSuccess() == 2) {
                                    this.dismiss();
                                    ToastUtil.toastAlerMessageCenter(GroupChatActivity.this, "发送中..请稍后重试..", 500);
                                } else {
...
//                                        if (viewItem.getIsSend()== BaseChatMsgEntity.TO_MSG)
                        ...
                        private void startTransmitActivity() {
                            Intent intent = new Intent(GroupChatActivity.this, TransmitActivity.class);
    ...
    private void initData() {
        ...
        myUserNo = LastLoginUserSP.getInstance(GroupChatActivity.this)
                .getUserAccount();
    ...
    public void findViews() {
        ...
        mAdapter = new ChatViewMsgAdapter(mContext, Const.CHAT_TYPE_GROUP, mChatUserNum);
    ...
    private void initGroupObject(boolean isUpdateGroup) {
        if (isUpdateGroup) {
            ...
            mGroupObject = (ContactGroup) getIntent().getSerializableExtra(Const.INTENT_GROUP_EXTRA_NAME);
        }
...
//					initGroupChatTitle();
    ...
    protected void onActivityResult(int requestCode, int resultCode, final Intent data) {
        //如果没有该群组，则发送按钮不可用
        if (mGroupObject == null) {
            ToastUtil.toastAlerMessageBottom(mContext, "暂无该群组,请手动更新联系人.", 800);
            return;
        }
     ...
     * @return
     */
    private GroupChatMsgEntity getEntityOfList(LinkedList<GroupChatMsgEntity> List,
                                               String packetId) {
    ...
    public void refreshUIByPageIndex(GroupChatMsgEntity deletedChatMsg) {
        isBottom = true;
        LinkedList<GroupChatMsgEntity> list = mGroupChatDao.getChatMsgEntitiesByPage(mChatUserNum, mPage, PAGE_SIZE);
    ...
    public void receivedMessage(GroupChatMsgEntity message) {
        ...
        mGroupChatDao.saveOrUpdate(message);
    ...
    private void clickSend() {
        ...
        body.setMsgType(Const.CHAT_TYPE_GROUP);
...
//			msg.setTo(JIDUtil.getSendToMsgAccount(mChatUserNum));
                    ...
                    if (NetWorkUtil.isNetworkAvailable(mContext)) {
                        ...
                        mHandler.postDelayed(new Runnable() {
                            ...
                            public void run() {
                                ...
                                mGroupChatDao.saveOrUpdate(groupChatMsgEntity);
            ...
            case GroupChatMsgEntity.IMAGE:
                ...
                if (groupChatMsgEntity.getIsSuccess() == GroupChatMsgEntity.SEND_ING) {
                    ImageUtil.reSendFailedImgChatMsgBean(GroupChatActivity.this,
                            groupChatMsgEntity, Const.CHAT_TYPE_GROUP, mChatUserNum,
                            mSendingListener);
                }
                break;
    ...
    private void notifyAdapterDataSetChanged(LinkedList<GroupChatMsgEntity> data) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            list.add(entity);
            mGroupChatDao.saveGroupChatMsg(entity);
        }
        //如果是第一页就刷新
        if (mPage == 0) {
            mMessageList.clear();
    ...
    public void onEventMainThread(BaseChatMsgEntity eventObj) {
        ...
        if (body.getMsgType() == Const.CHAT_TYPE_GROUP || body.getMsgType() == Const.CHAT_VOICE_TYPE_GROUP) {
            android.os.Message message = mHandler.obtainMessage();
    ...
    private void updateUnreadCount() {
        RecentChat recentChat = mRecentChatDao.isChatExist(mChatUserNum, Const.CHAT_TYPE_GROUP);
        if (recentChat != null) {
        ...
        mRecentChatDao.updateUnreadCount(mChatUserNum, Const.CHAT_TYPE_GROUP, 0);// 设置未读记录为0
     ...
     * 刷新UI
     */
    protected void updateChatMsgEntity(Message msg) {
        GroupChatMsgEntity chatMsg = new GroupChatMsgEntity();
        chatMsg.setPacketId(msg.getPacketID());
        chatMsg.setGroupId(mChatUserNum);
        chatMsg.setChatUserNo(mApplication.mSelfUser.getUserNo());

...
public class DragGridView extends GridView {
    ...
    private static final float AMP_FACTOR = 1.5f;
    ...
    private OnItemLongClickListener onLongClickListener = new OnItemLongClickListener() {
        ...
        public boolean onItemLongClick(AdapterView<?> parent, View view, int position, long id) {
            ...
            Bitmap dragBitmap = Bitmap.createBitmap(imageView.getDrawingCache());
            ...
            //设置透明度
            dragImageViewParams.alpha = 0.8f;
            //设置触摸点为绘制拖动item的中心
            ...
            //设置被长按item不显示
//            ((HomeAppGridAdapter) getAdapter()).hideView(position);
            return true;
        }
    };
...
//        params.height = getMeasuredHeight();
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        //dragImageView处于被拖动时，更新dragImageView位置
        else if ((ev.getAction() == MotionEvent.ACTION_MOVE) && (isViewOnDrag == true) && isEdit) {
            ...
            if ((currDraggedPosition != AdapterView.INVALID_POSITION) && (currDraggedPosition != preDraggedOverPositon)) {
                ((HomeAppGridAdapter) getAdapter()).swapView(preDraggedOverPositon, currDraggedPosition);
                preDraggedOverPositon = currDraggedPosition;
            }
        }
        //释放dragImageView
        else if ((ev.getAction() == MotionEvent.ACTION_UP) && (isViewOnDrag == true) && isEdit) {
//            ((HomeAppGridAdapter) getAdapter()).showHideView();
            //让父布局获取焦点

	...
	private void doClick() {
		if (mIsClick && mRecentChat != null) {
			...
			case Const.CHAT_TYPE_P2P:
				User user = mContactOrgDao.queryUserInfoByUserNo(mRecentChat.getUserNo());
				if (user!=null) {
					...
					ToastUtil.toastAlerMessage(mContext,"请删除失效用户数据!",500);
				}
				break;
...
//				groupintent.putExtra(P2PChatActivity.ACCOUNT,mRecentChat.getUserNo());
			...
			case Const.CHAT_TYPE_PLATFORM_NOTICE:
					Intent platNoticeIntent = new Intent(mContext,
							PlatformNoticeActivity.class);
//					Intent platNoticeIntent = new Intent(mContext,
//							TestActivity.class);
					mContext.startActivity(platNoticeIntent);
					break;
			default:
				break;
			}

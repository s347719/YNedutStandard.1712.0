...
public final class DefaultMsgConfig {
    ...
    public static class DownLoadMsgConfig  implements Parcelable {
        ...
        public boolean equals(Object mo) {
            ...
            if (!(mo instanceof DownLoadMsgConfig)) {
                return false;
            }

            DownLoadMsgConfig mthat = (DownLoadMsgConfig) mo;

            if (!getTaskHasBeenExist().equals(mthat.getTaskHasBeenExist())) {
                return false;
            }
            if (!getTips().equals(mthat.getTips())) {
                return false;
            }
            if (!getHoneycomblow().equals(mthat.getHoneycomblow())) {
                return false;
            }
            if (!getDownLoad().equals(mthat.getDownLoad())) {
                return false;
            }
            if (!getCancel().equals(mthat.getCancel())) {
                return false;
            }
            if (!getDownLoadFail().equals(mthat.getDownLoadFail())) {
                return false;
            }
            if (!getLoading().equals(mthat.getLoading())) {
                return false;
            }
            if (!getTrickter().equals(mthat.getTrickter())) {
                return false;
            }
            if (!getFileDownLoad().equals(mthat.getFileDownLoad())) {
                return false;
            }

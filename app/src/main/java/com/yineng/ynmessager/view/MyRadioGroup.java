...
public class MyRadioGroup extends LinearLayout {
	...
	public void addView(View child, int index, ViewGroup.LayoutParams params) {
		if (child instanceof RadioButton) {
			...
			//huyi
			View rootView = ((ViewGroup) child).getChildAt(0);
			final RadioButton button = findRadioButton((ViewGroup) rootView);
			if (button.isChecked()) {
				mProtectFromCheckedChange = true;
				if (mCheckedId != -1) {
					setCheckedStateForView(mCheckedId, false);
	...
	private class PassThroughHierarchyChangeListener implements
        ...
        public void onChildViewAdded(View parent, View child) {
			if (parent == MyRadioGroup.this && child instanceof RadioButton) {
				...
				//huyi
				View rootView = ((ViewGroup) child).getChildAt(0);
				RadioButton btn = findRadioButton((ViewGroup) rootView);
				int id = btn.getId();
				// generates an id if it's missing
				if (id == View.NO_ID) {
					id = btn.hashCode();
        ...
        public void onChildViewRemoved(View parent, View child) {
			if (parent == MyRadioGroup.this && child instanceof RadioButton) {
				...
				//huyi
				View rootView = ((ViewGroup) child).getChildAt(0);
				findRadioButton((ViewGroup) rootView).setOnCheckedChangeListener(
						null);
			}
			if (mOnHierarchyChangeListener != null) {

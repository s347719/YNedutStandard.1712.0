 ...
 * Descreption：可以根据需求有显示单行和非单行，会自动换行，可改变颜色和背景颜色
 */
...
public class TagCloudView  extends ViewGroup {
    ...
    private int mTextSpecialColor;
    private int mBackground;
    private int mSpecialBackground;
    ...
    private static final int DEFAULT_RIGHT_IMAGE = R.mipmap.contact_right_arrow;
    ...
    public TagCloudView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mTextSpecialColor = a.getColor(R.styleable.TagCloudView_tcvTextSpecialColor,DEFAULT_TEXT_COLOR);
        mBackground = a.getResourceId(R.styleable.TagCloudView_tcvBackground, DEFAULT_TEXT_BACKGROUND);
        mSpecialBackground =a.getResourceId(R.styleable.TagCloudView_tcvSpecialBackground,DEFAULT_TEXT_BACKGROUND);
    ...
    /**
     * tagList是展示的内容，只能是Item（可更改）根据里面的值随意更改
     * @param tagList
     */
    public void setTags(List<TagItem> tagList) {
        this.tags = tagList;
        this.removeAllViews();
        if (tags != null && tags.size() > 0) {
            ...
            for (int i = 0; i < tags.size(); i++) {
                ...
                if (tags.get(i).isRealTime()){
                    tagView.setBackgroundResource(mSpecialBackground);
                }else {
                    tagView.setBackgroundResource(mBackground);
                }
                if (tags.get(i).getColorCode()==1){
                    tagView.setTextColor(mTextSpecialColor);
                }else {
                    tagView.setTextColor(mTagColor);
                }
                LayoutParams layoutParams = new LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
                tagView.setLayoutParams(layoutParams);
                tagView.setText(tags.get(i).getValue());

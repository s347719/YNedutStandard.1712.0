...
public class DisGroupChatDao
{
	...
	public void deleteRecentChatByGroupId(String groupId)
	{
		mDb.delete(mTableName,"groupId = ?",new String[] {groupId});
	}
...
//				Log.i("groupMessage","message:   " + msg.getMessage());
		...
		for (int i = 0; i < list.size()-1; i++) {
			GroupChatMsgEntity tempObject = list.get(i);
			GroupChatMsgEntity tempObject1 = list.get(i+1);
			if (tempObject.getTime().equals(tempObject1.getTime())) {
				GroupChatMsgEntity swapObject = new GroupChatMsgEntity();
...
//				Log.i("groupMessage","message:   " + msg.getMessage());
	...
	// public synchronized TreeMap<Integer, GroupChatMsgEntity>
	// getChatMsgEntitiesToMap(String account) {
	// TreeMap<Integer, GroupChatMsgEntity> list = new TreeMap<Integer,
	// GroupChatMsgEntity>();
	...
	// msg.mIndex = cursor.getPosition();
	// Log.i("groupMessage", "message:   " + msg.getMessage()+" "+msg.mIndex);
	// // list.addFirst(msg);
	// // list.add(msg.mIndex,msg);
	// msg.mIndex = cursor.getPosition();
	// list.put(msg.mIndex, msg);
	// }
	// }
	// if (cursor != null) {
	// cursor.close();
	...
	/**
	 * 查询数据库中指定讨论组的最新（最后）一条消息记录
	 * 
	 * @param groupId 群组的groupId
	 * @return 返回指定讨论组的最新（最后）的一条消息记录对象；未查询到返回null
	 */
	 ...
	 * @param packetId
	 */
	public GroupChatMsgEntity queryInfoByPacketId(String packetId) {
		GroupChatMsgEntity chat = null;
		Cursor cur = mDb.query(mTableName,null,"packetId=?",new String[]{packetId},null,null,null);
		if(cur.moveToFirst())
		{
			chat = new GroupChatMsgEntity();
			chat.setPacketId(cur.getString(cur.getColumnIndex("packetId")));
	...
	public List<Object> querySearchResultByKeyWords(String mChatObjectId, String keyStr)
	{
		...
		Cursor cursor = mDb.query(mTableName,null,"groupId = ? and content like ? ESCAPE '/'",new String[] {
				mChatObjectId, "%" + keyStr + "%"},null,null,"mTime",null);
		if(cursor != null && cursor.getCount() > 0)
		{
			while(cursor.moveToNext())
			{
				...
				msg.setIsReaded(cursor.getInt(cursor.getColumnIndex("isReaded")));
				msg.setIsSuccess(cursor.getInt(cursor.getColumnIndex("isSuccess")));
//				Log.i("disgroupMessage","message:   " + msg.getMessage());
				list.addFirst(msg);
			}
	...
	public void updateMsgVoiceStatus(String packetId, int isReaded) {
		if (!mDb.isOpen()) {
            return;
        }
		if(IsExists(packetId))
		{
			ContentValues values = new ContentValues();
			values.put("isReaded",isReaded);
			String[] args = {packetId + ""};
			mDb.update(mTableName,values,"packetId = ?",args);
		}
	}

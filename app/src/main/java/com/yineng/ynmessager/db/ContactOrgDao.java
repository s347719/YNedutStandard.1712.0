...
import com.yineng.ynmessager.bean.contact.UserStatus.Resource;
...
{
	private String TAG = "ContactOrgDao";
...
//		L.e(mTAG +"(Context mContext, String Account) "+mDB.getPath());
	}
	...
	public ClientInitConfig getClientInitInfo()
	{
		...
		if(mCursor != null)
		{
			if(mCursor.getCount() == 1)
			{
				...
				int org_update_type = mCursor.getInt(mCursor.getColumnIndex("org_update_type"));
	...
	public void clearAllOrgData() {
		if (mDB.isOpen()) {
			mDB.delete(ORG_TREE_TABLE, null, null);
		}
	}
...
//		}
		mDB.insertWithOnConflict(ORG_TREE_TABLE, null, orgTreeContentValues, SQLiteDatabase.CONFLICT_REPLACE);
	}
	
	/**
	 * 插入或更新一条组织机构记录,如果存在就更新，否则插入
	 * 
	 * @param organizationTree
	 *            组织机构
	 */
	...
	{
		String[] resultColumn = new String[]{"orgNo"};
		Cursor mCursor = mDB.query(ORG_TREE_TABLE,resultColumn,"orgNo = ?",new String[] {orgId},null,null,null);
	...
	/**
	 * 通过递归查数据库来统计某组织机构的所有用户
	 */
	public List<User> getOrgUserByOrgIdFromDb(OrganizationTree mOrganizationTree,List<User> userList) {
		List<User> mUsers= queryUsersByOrgNo(mOrganizationTree.getOrgNo());
		if (mUsers!=null){
			userList.addAll(mUsers);
		}
		List<OrganizationTree> mList = queryOrgListByParentId(mOrganizationTree.getOrgNo());
		if (mList != null && mList.size() != 0) {
			for (OrganizationTree organizationTree : mList) {
				getOrgUserByOrgIdFromDb(organizationTree,userList);
			}
		}
		return mUsers;
	}
	...
	public List<OrganizationTree> queryOrgListByParentId(String parentOrgId)
	{
		...
		Cursor mCursor = mDB.query(ORG_TREE_TABLE,null,"removeTag = 0 and parentOrgNo = ?",new String[] {parentOrgId},null,null,
            "cast(ordId as double)");
        if(mCursor != null && mCursor.getCount() > 0)
		{
	 ...
	 * @return 结果按照ordId降序排列
	 */
	public List<String> queryOrgCountByParentId(String parentOrgId)
	{
		List<String> mList = null;
		Cursor mCursor = mDB.query(ORG_TREE_TABLE,new String[] {"orgNo"},"removeTag = 0 and parentOrgNo = ?",new String[] {parentOrgId},null,null,
				"ordId desc");
		if(mCursor != null && mCursor.getCount() > 0)
		{
	 ...
	 * @return 总人数
	 */
	public int getOrgUsersCountByOrgIdFromDb(OrganizationTree mOrganizationTree, HashSet<String> countSet)
	{
		HashSet<String> mUsers = queryUserCountByOrgNo(mOrganizationTree.getOrgNo());
		if(mUsers != null)
		{
			countSet.addAll(mUsers);
		}
		List<OrganizationTree> mList = queryOrgListByParentId(mOrganizationTree.getOrgNo());
		if(mList != null && mList.size() != 0)
		{
			for(OrganizationTree organizationTree : mList)
			{
				getOrgUsersCountByOrgIdFromDb(organizationTree, countSet);
			}
		}
			...
			ArrayList<OrganizationTree> mMyOrgPathList)
	{
		...
		{
			Cursor mCursor = mDB.query(ORG_TREE_TABLE,null,"removeTag = 0 and orgNo = ?",
	...
	public OrganizationTree queryUserRelationByUserNo(String mUserNo)
	{
		...
		String sql = "SELECT m.* FROM OrganizationTree m, UserTreeRelation n WHERE m.removeTag = 0 " +
				"and m.orgNo = n.orgNo AND n.removeTag = 0 AND n.userNo = ?";
				
		Cursor mCursor = mDB.rawQuery(sql,new String[] {mUserNo});
		if(mCursor != null && mCursor.getCount() > 0)
		{
	...
	public void clearAllUserData() {
		if (mDB.isOpen()) {
			mDB.delete(USER_TABLE, null, null);
		}
	}
...
//		mDB.beginTransaction();
		...
		{
			insertUpdateOneUserDataByAppInit(mUser);
		}
...
//		mDB.endTransaction();
	 ...
	 *            用户信息
	 */
	public void insertUpdateOneUserDataByAppInit(User mUser)
	{
//		boolean isUserExist = isUserExist(mUser.getUserNo());
		ContentValues userContentValues = new ContentValues();
		userContentValues.put("userNo",mUser.getUserNo());
...
//			mDB.insert(USER_TABLE,null,userContentValues);
//		}
		mDB.insertWithOnConflict(USER_TABLE, null, userContentValues, SQLiteDatabase.CONFLICT_REPLACE);
	}
	 ...
	 * @return
	 */
	public synchronized boolean isUserExist(String userNo)
	{
		String[] resultColumn = new String[]{"userNo"};
		Cursor mCursor = mDB.query(USER_TABLE,resultColumn,"userNo = ?",new String[] {userNo},null,null,null);
		if(mCursor != null && mCursor.getCount() > 0)
		{
			mCursor.close();
			return true;
	...
	public List<User> queryAllUser()
	{
		...
		Cursor mCursor = mDB.query(USER_TABLE,null,"removeTag = 0",null,null,null,null);
		if(mCursor != null && mCursor.getCount() > 0)
		{
	 ...
	 * @return
	 */
	public synchronized User queryUserInfoByUserNo(String mUserNo)
	{
		User tempUser = null;
		if (mDB == null) {
			String userAccount = LastLoginUserSP.getInstance(mContext).getUserAccount();
		...
		Cursor mCursor = mDB.query(USER_TABLE,null,"userNo = ? and removeTag = 0",new String[] {mUserNo},null,null,null);
		if(mCursor != null && mCursor.getCount() > 0)
		{
			while(mCursor.moveToNext())
			{
	...
	{
//		L.e("userStatus:userExist == "+isUserExist(mUserStatus.getUserNo()));
		//从User表中取出当前该用户
		User existUser = queryUserInfoByUserNo(mUserStatus.getUserNo());
		if(existUser != null)
		{
			ContentValues userContentValues = new ContentValues();
			List<Resource> resources = mUserStatus.getResources();
			//如果phone pc同时在线
			if (resources.size() > 1) {
				userContentValues.put("userStatus",Const.USER_ON_LINE_ALL);
			} else {
				if (resources.size() > 0) {
					Resource singleResource = resources.get(0);
					//如果phone上线
					if (singleResource.getResource().equals("Msg_Phone")) {
						userContentValues.put("userStatus",Const.USER_ON_LINE_PHONE);
					} 
					//如果PC上线
					else if (singleResource.getResource().equals("Msg_PC")) {
						userContentValues.put("userStatus",Const.USER_ON_LINE_PC);
					}
				}
			}
	...
	public void clearAllUserOrgRelationData() {
		if (mDB.isOpen()) {
			mDB.delete(USER_TREE_RELATION_TABLE, null, null);
		}
	}
...
//		mDB.endTransaction();
	...
	public void insertUpdateOneUserRelationData(UserTreeRelation mUserRelation)
	{
		...
		mDB.insertWithOnConflict(USER_TREE_RELATION_TABLE, null, userContentValues, SQLiteDatabase.CONFLICT_REPLACE);
	}
	
	/**
	 * 插入或更新一个用户机构关系信息,存在就更新，否则插入
	 * 
	 * @param mUserRelation
	 *            用户机构关系信息
	 */
	private void insertUpdateUserRelDataByExist(UserTreeRelation mUserRelation)
	{
	...
	public List<User> queryUsersByOrgNo(String orgNo)
	{
        ...
        String userNo = LastLoginUserSP.getLoginUserNo(mContext);
        Cursor mCursor = mDB.rawQuery("SELECT u.*,case when userStatus=2 then 1 when u.userNo='"
            + userNo
            + "' then 3 else userStatus end userStatus_order FROM "
            + USER_TREE_RELATION_TABLE
            + " m,"
            + USER_TABLE
            + " u WHERE m.userNo = u.userNo AND m.removeTag = 0 AND m.orgNo = '"
            + orgNo
            + "'"
            + " order by userStatus_order desc,cast(ordId as double) asc", null);
        if(mCursor != null && mCursor.getCount() > 0)
		{
	 ...
	 * @return 组织机构下的人
	 */
	public HashSet<String> queryUserCountByOrgNo(String orgNo)
	{
		HashSet<String> mUserNos = null;
		Cursor mCursor = mDB.rawQuery("SELECT u.userNo FROM " + USER_TREE_RELATION_TABLE + " m," + USER_TABLE
            + " u WHERE m.userNo = u.userNo AND m.removeTag = 0 AND m.orgNo = '"
            + orgNo
            + "'"
            + " order by userStatus desc,cast(ordId as double) asc", null);
        if(mCursor != null && mCursor.getCount() > 0)
		{
            mUserNos = new HashSet<>();
            while(mCursor.moveToNext())
			{
				String mUserNo = mCursor.getString(mCursor.getColumnIndex("userNo"));
				//修复组织机构，一个用户在一棵树下的不同层级节点下，应不记重复数的bug
				mUserNos.add(mUserNo);
			}
		}
		if(mCursor != null)
		{
...
//		mDB.endTransaction();
	...
	public void clearAllUserStatus()
	{
		...
		ContentValues clearUserStatusCV = new ContentValues();
		clearUserStatusCV.put("userStatus", Const.USER_OFF_LINE);
		mDB.update(USER_TABLE, clearUserStatusCV, null, null);
	}
	...
	{
		//保存删除群组表之前的消息提醒设置到最新获取的群组数据里
		List<ContactGroup> dbGroups = queryGroupList(groupType);
		if (dbGroups.size() > 0) {
			for (ContactGroup dbGroup : dbGroups) {
				for (ContactGroup newGroup : mList) {
					if (newGroup.getGroupName().equals(dbGroup.getGroupName())) {
						newGroup.setNotifyMode(dbGroup.getNotifyMode());
						break;
					}
				}
			}
		...
		//add by huyi 2015.9.15 for 优化sql数据插入
		StringBuilder sqlBuilder = new StringBuilder("INSERT INTO ");
		if (groupType == Const.CONTACT_GROUP_TYPE) {
			sqlBuilder.append(CONTACT_GROUP);
		} else {
			sqlBuilder.append(CONTACT_DIS_GROUP);
		}
		sqlBuilder.append(" VALUES(?,?,?,?,?,?,?,?)");
		SQLiteStatement stmt = mDB.compileStatement(sqlBuilder.toString());
		for(ContactGroup contactGroup : mList)
		{
//			insertOneContactGroupData(contactGroup,groupType);
			insertOneGroupData(stmt,contactGroup);
		}
	}
	 ...
	 * 插入一条群组/讨论组记录
	 * @param stmt
	 * @param contactGroup 群/讨论组数据
	 */
	private void insertOneGroupData(SQLiteStatement stmt, ContactGroup contactGroup) {
		stmt.clearBindings();
		if (contactGroup.getGroupName() != null) {
			stmt.bindString(1,contactGroup.getGroupName());
		} else {
			stmt.bindNull(1);
		}
		
		if (contactGroup.getCreateUser() != null) {
			stmt.bindString(2,contactGroup.getCreateUser());
		} else {
			stmt.bindNull(2);
		}
		
		if (contactGroup.getCreateTime() != null) {
			stmt.bindString(3,contactGroup.getCreateTime());
		} else {
			stmt.bindNull(3);
		}
		if (contactGroup.getNaturalName() != null) {
			stmt.bindString(4,contactGroup.getNaturalName());
		} else {
			stmt.bindNull(4);
		}
		if (contactGroup.getSubject() != null) {
			stmt.bindString(5,contactGroup.getSubject());
		} else {
			stmt.bindNull(5);
		}

		stmt.bindLong(6,contactGroup.getMaxUsers());
		
		if (contactGroup.getDesc() != null) {
			stmt.bindString(7,contactGroup.getDesc());
		} else {
			stmt.bindNull(7);
		}
		stmt.bindLong(8,contactGroup.getNotifyMode());
		stmt.executeInsert();
	}
	 ...
	 *            群/讨论组数据
	 */
	public void insertOrUpdateOneContactGroupData(ContactGroup contactGroup, int groupType)
	{
	...
	public synchronized void saveAllGroupUserRelationData(List<ContactGroupUser> mList, int groupType)
	{
		...
		StringBuilder sqlBuilder = new StringBuilder("INSERT INTO ");
		if (groupType == Const.CONTACT_GROUP_TYPE) {
			sqlBuilder.append(CONTACT_GROUP_USER_REL);
		} else {
			sqlBuilder.append(CONTACT_DIS_GROUP_USER_REL);
		}
		sqlBuilder.append("('userNo','groupName','role')");
		sqlBuilder.append(" VALUES(?,?,?)");
		SQLiteStatement stmt = mDB.compileStatement(sqlBuilder.toString());
		for (int i = 0; i < mList.size(); i++) {
			ContactGroupUser mContactGroupUser = mList.get(i);
			insertOneGroupUserRelData(stmt,mContactGroupUser);
		}
...
//		}
	 ...
	 * 插入一个群/讨论组成员关系信息
	 * @param stmt
	 * @param mContactGroupUser  一个用户和群关系的实例
	 */
	private void insertOneGroupUserRelData(SQLiteStatement stmt, ContactGroupUser mContactGroupUser) {
		String mUserNO;
		...
		stmt.clearBindings();
		if (mUserNO != null) {
			stmt.bindString(1,mUserNO);
		} else {
			stmt.bindNull(1);
		}
		
		if (mContactGroupUser.getGroupName() != null) {
			stmt.bindString(2,mContactGroupUser.getGroupName());
		} else {
			stmt.bindNull(2);
		}
		
		stmt.bindString(3,mContactGroupUser.getRole()+"");
		stmt.executeInsert();
	}
	...
	public void insertOneGroupUserRelationData(ContactGroupUser mContactGroupUser, int groupType)
	{
		...
		groupUserCV.put("role",mContactGroupUser.getRole()); // 群成员类型 10-创建人 20-管理员  50-一般用户
		if (groupType == 8) {
	 ...
	 *            8：群组 9：讨论组
	 */
	public void insertOrUpdateOneGroupUserRelationData(ContactGroupUser mContactGroupUser, int groupType)
	{
		String mUserNO;
		if(mContactGroupUser.getJid().contains("@"))
		{
    ...
    /**
     * @param userNo 当前登录的用户id
     * @return 我创建的讨论组数量。异常返回-1
     */
    public int queryGroupSumCreatedByMe(String userNo)
    {
/*		Cursor mCursor = mDB.query(CONTACT_DIS_GROUP_USER_REL,null,"userNo = ? and role = ?",new String[] {userNo,
                "10"},null,null,null);
		if (mCursor != null && mCursor.getCount() > 0) {
        ...
        int count = -1;
        String sql = String.format("select count(id) from (select id,userNo,role from %s where userNo=? and role=?)",
                CONTACT_DIS_GROUP_USER_REL);
        Cursor cur = mDB.rawQuery(sql, new String[]{userNo, "10"});
        if (cur.moveToFirst())
        {
            count = cur.getInt(0);
        }
	...
	public List<User> queryUsersByGroupName(String groupName, int groupType)
	{
		...
		if(groupType == 8)
		{
			mCursor = mDB.rawQuery("SELECT u.* FROM " + CONTACT_GROUP_USER_REL + " m," + USER_TABLE
					+ " u WHERE m.userNo = u.userNo and u.removeTag = 0 AND m.groupName = '" + groupName + "'",null);
		}else
		{
	...
	public List<OrganizationTree> queryOrgListByKeyWords(String keyStr)
	{
		...
		Cursor mCursor = mDB.query(ORG_TREE_TABLE, null, "removeTag = 0 and orgName like ? ESCAPE '/'", new String[]{"%" + keyStr + "%"},
	...
	public List<User> queryUserListByKeyWords(String keyStr)
	{
		...
		keyStr = keyStr.replaceAll("_","/_");
		keyStr = keyStr.replaceAll("%","/%");
		Cursor mCursor = mDB.query(USER_TABLE,null,"removeTag = 0 and userName like ? ESCAPE '/'",new String[] {"%" + keyStr + "%"},null,
	...
	/**
	 * 在查找聊天信息中根据发送消息的人名来查找发送者的性别状态
	 * @param name
	 * @return
	 */
	public List<User> queryUserLisByChatName(String name){
		List<User> mList = null;
		Cursor mCursor = mDB.query(USER_TABLE,null,"removeTag = 0 and userName =?",new String[] {name},null,
				null,null);
		if(mCursor != null && mCursor.getCount() > 0)
		{
			mList = new ArrayList<User>();
			...
			{
				int gender = mCursor.getInt(mCursor.getColumnIndex("gender"));
				User tempUser = new User("","",gender,"","","","","","", 0,0,0);
				mList.add(tempUser);
			}
		}
		if(mCursor != null)
	...
	public List<ContactGroup> queryGroupListByKeyWords(String keyStr)
	{
		...
		for(int groupType = 8; groupType < 10; groupType++)
		{
			...
			{
				mCursor = mDB.query(CONTACT_DIS_GROUP,null,"subject like ? ESCAPE '/'",new String[] {"%" + keyStr
						+ "%"},null,null,null);
			}
			if(mCursor != null && mCursor.getCount() > 0)
			{
	 ...
	 * @param allContactOrg
	 */
	public boolean saveAllContactOrg(ContactOrg allContactOrg) {
		boolean saveSuccess = true;
		try {
			saveAllOrgData(allContactOrg.getOrgList());
			saveAllUserData(allContactOrg.getUserList());
			saveAllUserRelationData(allContactOrg.getRelList());
		} catch (Exception e) {
			e.printStackTrace();
			saveSuccess = false;
		}
	...
	/**
	 * 保存联系人信息
	 * @param allContactOrg 组织机构
	 * @param ORG_UPDATE_TYPE 更新方式,0 全量  1 增量
	 * @return
	 */
	public synchronized boolean saveAllContactOrg(ContactOrg allContactOrg, int ORG_UPDATE_TYPE) {
		boolean saveSuccess = true;
		if (!mDB.isOpen()) {
            return false;
        }
		//如果是全量更新，把数据库的组织机构数据全部删除
		if (ORG_UPDATE_TYPE == Const.ORG_UPDATE_ALL) {
			clearAllOrgData();
			clearAllUserData();
		...
		try {
			mDB.beginTransaction();
			if (allContactOrg.getOrgList().size()>0) {
				saveOrgDataByUpdateType(allContactOrg.getOrgList(), ORG_UPDATE_TYPE);
			}
			if (allContactOrg.getUserList().size()>0) {
				saveUserDataByUpdateType(allContactOrg.getUserList(), ORG_UPDATE_TYPE);
			}
			if (allContactOrg.getRelList().size()>0) {
				saveUserOrgRelDataByUpdateType(allContactOrg.getRelList(), ORG_UPDATE_TYPE);
			}
		} catch (Exception e) {
			e.printStackTrace();
			saveSuccess = false;
	 ...
	 * 根据组织机构更新方式保存组织机构数据
	 * @param orgList
	 * @param ORG_UPDATE_TYPE
	 */
	private synchronized void saveOrgDataByUpdateType(List<OrganizationTree> orgList,int ORG_UPDATE_TYPE) {
		if (ORG_UPDATE_TYPE == Const.ORG_UPDATE_ALL) {
			//add by huyi 2015.10.28 for 优化sql数据插入
			StringBuilder sqlBuilder = new StringBuilder("INSERT INTO "+ORG_TREE_TABLE);
			sqlBuilder.append(" VALUES(?,?,?,?,?,?)");
			SQLiteStatement stmt = mDB.compileStatement(sqlBuilder.toString());
//			mDB.beginTransaction();
			for(OrganizationTree contactOrg : orgList)
			{
				L.e("MSG",contactOrg.getOrgNo());
				insertOneOrgDataByAll(stmt,contactOrg);
			}
//			mDB.setTransactionSuccessful();
//			mDB.endTransaction();
			stmt.close();
		} else {
			for(OrganizationTree organizationTree : orgList){
				insertUpdateOrgDataByExist(organizationTree);
			}
		}
	}
	 ...
	 * 插入一条组织机构记录
	 * @param stmt
	 * @param contactOrg 组织机构记录
	 */
	private synchronized void insertOneOrgDataByAll(SQLiteStatement stmt, OrganizationTree contactOrg) {
		stmt.clearBindings();
		if (contactOrg.getOrgNo() != null) {
			stmt.bindString(1,contactOrg.getOrgNo());
		} else {
			stmt.bindNull(1);
		}
		
		if (contactOrg.getParentOrgNo() != null) {
			stmt.bindString(2,contactOrg.getParentOrgNo());
		} else {
			stmt.bindNull(2);
		}
		
		if (contactOrg.getOrgName() != null) {
			stmt.bindString(3,contactOrg.getOrgName());
		} else {
			stmt.bindNull(3);
		}
		stmt.bindLong(4,contactOrg.getOrgType());
		
		if (contactOrg.getOrdId()!= null) {
			stmt.bindString(5,contactOrg.getOrdId());
		} else {
			stmt.bindNull(5);
		}

		stmt.bindLong(6,contactOrg.getRemoveTag());
		stmt.executeInsert();
	}

	/**
	 * 根据组织机构更新方式保存组织机构用户数据
	 * @param userList
	 * @param ORG_UPDATE_TYPE
	 */
	private synchronized void saveUserDataByUpdateType(List<User> userList, int ORG_UPDATE_TYPE) {
		if (ORG_UPDATE_TYPE == Const.ORG_UPDATE_ALL) {
			//add by huyi 2015.10.28 for 优化sql数据插入
			StringBuilder sqlBuilder = new StringBuilder("INSERT INTO "+USER_TABLE);
			sqlBuilder.append(" VALUES(?,?,?,?,?,?,?,?,?,?,?,?)");
			SQLiteStatement stmt = mDB.compileStatement(sqlBuilder.toString());
			for(User contactUser : userList)
			{
				insertOneOrgUserDataByAll(stmt,contactUser);
			}
			stmt.close();
		} else {
	 ...
	 * @param contactUser
	 */
	private synchronized void insertOneOrgUserDataByAll(SQLiteStatement stmt, User contactUser) {
		stmt.clearBindings();
		if (contactUser.getUserNo() != null) {
			stmt.bindString(1,contactUser.getUserNo());
		} else {
			stmt.bindNull(1);
		}
		if (contactUser.getUserName() != null) {
			stmt.bindString(2,contactUser.getUserName());
		} else {
			stmt.bindNull(2);
		}
		stmt.bindLong(3,contactUser.getGender());
		if (contactUser.getDayOfBirth() != null) {
			stmt.bindString(4,contactUser.getDayOfBirth());
		} else {
			stmt.bindNull(4);
		}
		if (contactUser.getTelephone()!= null) {
			stmt.bindString(5,contactUser.getTelephone());
		} else {
			stmt.bindNull(5);
		}
		if (contactUser.getEmail()!= null) {
			stmt.bindString(6,contactUser.getEmail());
		} else {
			stmt.bindNull(6);
		}
		if (contactUser.getPost()!= null) {
			stmt.bindString(7,contactUser.getPost());
		} else {
			stmt.bindNull(7);
		}
		if (contactUser.getHeadUrl()!= null) {
			stmt.bindString(8,contactUser.getHeadUrl());
		} else {
			stmt.bindNull(8);
		}
		if (contactUser.getSigature()!= null) {
			stmt.bindString(9,contactUser.getSigature());
		} else {
			stmt.bindNull(9);
		}
		stmt.bindLong(10,contactUser.getUserType());
		stmt.bindLong(11,contactUser.getUserStatus());
		stmt.bindLong(12,contactUser.getRemoveTag());
		stmt.executeInsert();
	}
	
	/**
	 * 根据组织机构更新方式保存用户与组织机构关系数据
	 * @param relList
	 * @param ORG_UPDATE_TYPE
	 */
	private synchronized void saveUserOrgRelDataByUpdateType(List<UserTreeRelation> relList, int ORG_UPDATE_TYPE) {
		if (ORG_UPDATE_TYPE == Const.ORG_UPDATE_ALL) {
			//add by huyi 2015.10.28 for 优化sql数据插入
			StringBuilder sqlBuilder = new StringBuilder("INSERT INTO "+USER_TREE_RELATION_TABLE);
			sqlBuilder.append(" VALUES(?,?,?,?,?)");
			SQLiteStatement stmt = mDB.compileStatement(sqlBuilder.toString());
			for(UserTreeRelation userOrgRel : relList)
			{
				insertOneOrgUserRelDataByAll(stmt,userOrgRel);
			}
			stmt.close();
		} else {
			for(UserTreeRelation mUserRelation : relList)
			{
				insertUpdateUserRelDataByExist(mUserRelation);
			}
		}
	}
	 ...
	 * @param userOrgRel
	 */
	private synchronized void insertOneOrgUserRelDataByAll(SQLiteStatement stmt, UserTreeRelation userOrgRel) {
		stmt.clearBindings();
		if (userOrgRel.getUserNo() != null) {
			stmt.bindString(1,userOrgRel.getUserNo());
		} else {
			stmt.bindNull(1);
		}
		if (userOrgRel.getOrgNo() != null) {
			stmt.bindString(2,userOrgRel.getOrgNo());
		} else {
			stmt.bindNull(2);
		}
		if (userOrgRel.getOrdId() != null) {
			stmt.bindString(3,userOrgRel.getOrdId());
		} else {
			stmt.bindNull(3);
		}
		if (userOrgRel.getRelationType()!= null) {
			stmt.bindString(4,userOrgRel.getRelationType());
		} else {
			stmt.bindNull(4);
		}
		stmt.bindLong(5,userOrgRel.getRemoveTag());
		stmt.executeInsert();
	}

	/**
	 * 保存群 讨论组信息
	 * @param mContactGroupList
	 */
	public synchronized boolean saveAllGroupDatas(ArrayList<ContactGroupBean> mContactGroupList) {
		boolean saveSuccess = true;
		try {
			if (!mDB.isOpen()) {
                return false;
			...
			mDB.beginTransaction();
			for (int i = 0; i < mContactGroupList.size(); i++) {
				ContactGroupBean contactGroupBean = mContactGroupList.get(i);
				saveAllGroupAndUserRelData(contactGroupBean.getRoomList(),
						contactGroupBean.getUserList(),contactGroupBean.getGroupType());
			}
		} catch (Exception e) {
			e.printStackTrace();
			saveSuccess = false;
	 ...
	 * @param roomList 群组数据
	 * @param userList 成员数据
	 * @param contactGroupType 群或讨论组
	 */
	public synchronized void saveAllGroupAndUserRelData(List<ContactGroup> roomList,
			List<ContactGroupUser> userList, int contactGroupType) {
		saveAllContactGroupData(roomList, contactGroupType);
		saveAllGroupUserRelationData(userList, contactGroupType);
	}

	public synchronized void saveOrUpdateContactGroup(ContactGroupBean mContactGroupBean) {
		for (ContactGroup contactGroup : mContactGroupBean.getRoomList()) {
			insertOrUpdateOneContactGroupData(contactGroup, mContactGroupBean.getGroupType());
		}
		for (ContactGroupUser mContactGroupUser : mContactGroupBean.getUserList()) {
			insertOrUpdateOneGroupUserRelationData(mContactGroupUser, mContactGroupBean.getGroupType());
		}
	}
}

...
//	/**
//	 * 直接返回当前的UserAccountDB对象
//	 * @return 若当前UserAccountDB还未被初始化，则会返回null
//	 */
//	public static synchronized UserAccountDB getInstance()
//	{
//		return UserAccountDB;
//	}
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        //用户设置表
        sql = SettingsTb.getStructureSql();
        L.d(sql);
        db.execSQL(sql);

        //事件表
        sql = NoticeEventTb.getStructureSql();
        L.d(sql);
        db.execSQL(sql);

        //应用表
        sql = getNewMyAppsTableSql();
        L.d(sql);
        db.execSQL(sql);

        //待办表
        sql = getDealtTableSql();
        L.d(sql);
        db.execSQL(sql);

        //推广投放
        sql = getPublicizeTableSql();
        L.d(sql);
        db.execSQL(sql);

        //gps定位信息表
        sql = LocationsTb.getStructureSql();
        L.d(sql);
        db.execSQL(sql);


        //下载文件信息表
        sql = getDownLoadFileSql();
        L.d(sql);
        db.execSQL(sql);
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        L.d(TAG, "database onUpgrade -> oldVersion: " + oldVersion + ", newVersion: " + newVersion);
        //版本差 如果当前版本号是7,版本差=3 就更新4,5,6,7号数据库
        int version = newVersion - oldVersion;
        switch (version) {
            case 4:
                if (newVersion == 7) {
                    doUpgradeV4(db);
...
//
     ...
     * 升级数据库,保留以前的数据
     */
    private void doUpgradeV3(SQLiteDatabase db) {
        String createTempTable = "alter table " + NoticeEventTbDao.TABLE_NAME + " rename to tempNotice";
        String createTable = NoticeEventTb.getStructureSql();
        String insertData = "insert into " + NoticeEventTbDao.TABLE_NAME + " select *,\"\",\"\",\"\",\"\" from tempNotice";
        String dropTempTable = "drop table tempNotice";

        L.v(TAG, createTempTable);
        db.execSQL(createTempTable);
        L.v(TAG, createTable);
        db.execSQL(createTable);
        L.v(TAG, insertData);
        db.execSQL(insertData);
        L.v(TAG, dropTempTable);
        db.execSQL(dropTempTable);
    }

    private void doUpgradeV4(SQLiteDatabase db) {
        // 应用表添加SourceSys字段
        String sql = String.format("ALTER TABLE %s ADD COLUMN %s INTEGER",
                MyAppsTbDao.TABLE_NAME,
                MyAppsTbDao.COLUMN_SOURCE_SYS);
     ...
     * @param db
     */
    private void doUpgradeV5(SQLiteDatabase db) {
        String sql = "";
        //删除以前的表
        db.execSQL("DROP TABLE MyApps");
        //修改用户设置表，增加两个字段
        sql = "ALTER TABLE " + SettingsTb.TABLE_NAME + " ADD COLUMN " +
                SettingsTb.COLUMN_IS_RECOMMEND_APP +
                " INTEGER DEFAULT " + SettingsTb.VALUE_RECOMMEND_APP;
        db.execSQL(sql);

        sql = "ALTER TABLE " + SettingsTb.TABLE_NAME + " ADD COLUMN " +
                SettingsTb.COLUMN_IS_RECOMMEND_APP_DIALOG_SHOW +
                " INTEGER DEFAULT " + SettingsTb.VALUE_RECOMMEND_APP_DIALOG;
        db.execSQL(sql);
     ...
     * @param db
     */
    private void doUpgradeV6(SQLiteDatabase db) {
    ...
    /**
     * 升级数据库到7 定位数据增加一个用户ID
     *
     * @param db
     */
    private void doUpgradeV7(SQLiteDatabase db) {

        // 定位数据表增加 rsglSysUserId 字段
        String sql = String.format("ALTER TABLE %s ADD COLUMN %s TEXT",
                LocationsTbDao.TABLE_NAME,
                LocationsTbDao.COLUMN_RSGSYSUSERID);
    ...
    private String getClientInitTableSql() {
        ...
        sb_ClientInit.append("CREATE TABLE IF NOT EXISTS [ClientInit]("); // 表名
    ...
    private String getOrganizationTreeTableSql() {
        ...
        sb_OrgTree.append("CREATE TABLE IF NOT EXISTS [OrganizationTree]("); // 表名
        ...
        sb_OrgTree.append("[ordId] varchar(36),"); // 排序
    ...
    private String getUserTableSql() {
        ...
        sb_User.append("CREATE TABLE IF NOT EXISTS [User]("); // 表名
        ...
        sb_User.append("[userStatus] INTEGER default" + Const.USER_OFF_LINE + ","); // 用户在线状态
    ...
    private String getUserTreeRelationTableSql() {
        ...
        sb_UserTreeRelation.append("CREATE TABLE IF NOT EXISTS [UserTreeRelation]("); // 表名
//		sb_UserTreeRelation.append("[id] INTEGER PRIMARY KEY AUTOINCREMENT,"); // Id
        ...
        sb_UserTreeRelation.append("[ordId] varchar(36),"); // 排序
    ...
    private String getUserStatusTableSql() {
        ...
        sb_UserStatus.append("CREATE TABLE IF NOT EXISTS [UserStatus]("); // 表名
    ...
    private String getUserGroupTableSql() {
        ...
        sb_userGroup.append("CREATE TABLE IF NOT EXISTS [UserGroup]("); // 表名
        ...
        sb_userGroup.append("[notifyMode] INTEGER default " + ContactGroup.NOTIFYMODE_NOTIFY + ")");  //群的消息提醒方式，默认接收提醒
    ...
    private String getDiscussGroupTableSql() {
        ...
        sb_discussGroup.append("CREATE TABLE IF NOT EXISTS [DiscussGroup]("); // 表名
        ...
        sb_discussGroup.append("[notifyMode] INTEGER default " + ContactGroup.NOTIFYMODE_NOTIFY + ")");  //讨论组消息提醒方式，默认接收提醒
    ...
    private String getUserGroupRelationsTableSql() {
        ...
        sb_userGroupRel.append("CREATE TABLE IF NOT EXISTS [UserGroupRelations]("); // 表名
        sb_userGroupRel.append("[id] INTEGER PRIMARY KEY,"); // 主键自增
    ...
    private String getUserDisGroupRelationTableSql() {
        ...
        sb_userDisGroupRel
                .append("CREATE TABLE IF NOT EXISTS [UserDiscussionGroupRelations]("); // 表名
        sb_userDisGroupRel.append("[id] INTEGER PRIMARY KEY,"); // 主键自增
    ...
    private String getUserGroupFileShareTableSql() {
        ...
        sb_userFileShare.append("CREATE TABLE IF NOT EXISTS [UserGroupFileShare]("); // 表名
    ...
    private String getTwoChatTableSql() {
        ...
        sb_twoChat.append("CREATE TABLE IF NOT EXISTS [Chat]("); // 表名
    ...
    private String getGroupChatTableSql() {
        ...
        sb_groupChat.append("CREATE TABLE IF NOT EXISTS [GroupChat]("); // 表名
    ...
    private String getDiscussionChatTableSql() {
        ...
        sb_disChat.append("CREATE TABLE IF NOT EXISTS [DiscussionChat]("); // 表名
    ...
    private String getDroadcastChatTableSql() {
        ...
        sb_droadcastChat.append("CREATE TABLE IF NOT EXISTS [BroadcastChat]("); // 表名
	 ...
	 * new StringBuffer(); sb_recentChat.append("CREATE TABLE IF NOT EXISTS [RecentChat](");
    ...
    private String getRecentChatTableSql() {
        ...
        sb_recentChat.append("CREATE TABLE IF NOT EXISTS [RecentChat]("); // 表名
        ...
        sb_recentChat.append("[senderNo] varchar(30),"); // 发送者帐号：自己或对方（个人对话时，senderNo==fromUserNo）
    ...
    // sb_unreadChat.append("CREATE TABLE IF NOT EXISTS [UnreadChat]("); // 表名
    ...
    private String getFileAttrTableSql() {
        ...
        sb_fileAttr.append("CREATE TABLE IF NOT EXISTS [FileAttr]("); // 表名
     ...
     * @return 新app表
     */
    private String getNewMyAppsTableSql() {
        StringBuffer sb = new StringBuffer();
        sb.append("CREATE TABLE IF NOT EXISTS [NewMyApps] (");
        sb.append("   [id]                   varchar primary key,"); // 应用id
        sb.append("   [sortFieldMap]         varchar,"); //
        sb.append("   [name]                 varchar,"); //应用名称
        sb.append("   [source]               integer,");
        sb.append("   [is_new]               integer,"); //是否是新添加的app 1:最新；0:不是最新
        sb.append("   [appTypeId]            varchar,"); //app类型id
        sb.append("   [appTypeName]          varchar,"); //app类型名称
        sb.append("   [icon]                 varchar,"); //图标名称
        sb.append("   [sequence]             integer,"); //排序编号
        sb.append("   [topSequence]          integer,"); //推荐排序编号
        sb.append("   [typeSequence]         integer,"); //分类排序
        sb.append("   [last_use_date]        varchar,"); //最后使用时间
        sb.append("   [isRecommend]          integer,"); //是否推荐
        sb.append("   [submenu]              varchar,"); //子菜单，存json
        sb.append("   [updateTime]           varchar,"); //应用更新时间
        sb.append("   [user_delete]          integer DEFAULT 1,"); //用户是否删除应用
        sb.append("   [url]                  varchar )"); //链接地址
        return sb.toString();
    }
     ...
     * @return 待办表
     */
    private String getDealtTableSql() {
        StringBuffer sb = new StringBuffer();
        sb.append("CREATE TABLE IF NOT EXISTS [Dealt] (");
        sb.append("   [id]                   varchar primary key,"); // 应用id
        sb.append("   [name]                 varchar,"); //待办名称
        sb.append("   [count]                varchar,"); //待办数量
        sb.append("   [sequence]             integer )"); //待办排序
        return sb.toString();
    }
     ...
     * @return 推广投放
     */
    private String getPublicizeTableSql() {
        StringBuffer sb = new StringBuffer();
        sb.append("CREATE TABLE IF NOT EXISTS [Publicize] (");
        sb.append("   [id]                   varchar primary key,"); // id
        sb.append("   [title]                varchar,"); //名称
        sb.append("   [method]               integer,"); //投放方式
        sb.append("   [remark]               varchar,"); //投放文字内容
        sb.append("   [sequence]             integer,"); //排序
        sb.append("   [url]                  varchar,"); //跳转地址
        sb.append("   [fileName]             varchar,"); //图片名称
        sb.append("   [sortFieldMap]         varchar,");
        sb.append("   [isRead]               integer,"); //是否已读0：未读；1：已读
        sb.append("   [filePath]             varchar)"); //图片下载路径
        return sb.toString();
    }
    ...
    public static String getDownLoadFileSql() {
        StringBuffer sb_recentChat = new StringBuffer();
        sb_recentChat.append("CREATE TABLE IF NOT EXISTS [DownLoadFile]("); // 表名
        sb_recentChat.append("[id] INTEGER PRIMARY KEY AUTOINCREMENT,");// 主键,自增Id
        sb_recentChat.append("[fileName] text,"); // 图片的文件名
        sb_recentChat.append("[fileId] varchar(100),"); // 当前会话的ID
        sb_recentChat.append("[fileSource] INTEGER DEFAULT -1,");  //当前保存图片会话类型   0-个人会话  1-讨论组   2-群组
        sb_recentChat.append("[dataTime] timestamp,"); //   图片保存时间（取数据保存的时间）
        sb_recentChat.append("[isSend] INTEGER DEFAULT -1,"); // 0:是发送 1:不是发送（即接收）
        sb_recentChat.append("[fileType] INTEGER DEFAULT -1,"); //下载文件类型在这里都是图片为 1    1-图片  2-文件
        sb_recentChat.append("[sendUserNo] varchar(100),"); // 发送者帐号：userNo
        sb_recentChat.append("[packetId] varchar(100),"); //下载文件在聊天记录中的packetId 唯一标记
        sb_recentChat.append("[size] varchar(100))"); //下载文件大小
        return sb_recentChat.toString();
    }

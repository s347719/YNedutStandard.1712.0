...
exports.open = function(uri, success, error, trustAllCertificates) {
  if (!uri || arguments.length === 0) { return false; }
    var imageReg=/^(bmp|jpg|jpeg|png|tiff|gif|pcx|tga|exif|fpx|svg|psd|cdr|pcd|dxf|ufo|eps|ai|raw|wmf)$/;
    var ext = uri.substring(uri.lastIndexOf('.') + 1).toLocaleLowerCase();
  if (uri.match('http') && !imageReg.test(ext)) {
